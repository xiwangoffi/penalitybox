...
var imminanceFeuxVert = null;
var feuxRougeAllume = false;

class LancementPenalite extends Component {
    constructor(props) {
        super(props);
    ...
    pageSuivante() {
        if (this.props.route.params.page === 1) {
            fetch(urlBase + '/extinction/feux').then(donnees => {
                // Faire clignoter les feux toutes les secondes
                imminanceFeuxVert = setInterval(() => {
                    if (feuxRougeAllume) {
                        feuxRougeAllume = false;
                        fetch(urlBase + '/extinction/feux');
                    } else {
                        feuxRougeAllume = true;
                        fetch(urlBase + '/allumer/feux/rouge');
                    }
                }, 1000);
                this.props.navigation.push('LancementPenalite', { headerTitle: this.props.route.params.headerTitle, texte: "Durée de l'imminance du feux vert :", page: 2, temps: this.props.route.params.tempsImminanceFeuxVert, tempsFeuxVert: this.props.route.params.tempsFeuxVert, mode: this.props.route.params.mode });
            });
        } else if (this.props.route.params.page === 2) {
            clearInterval(imminanceFeuxVert);
            fetch(urlBase + '/extinction/feux').then(donnees => {
                fetch(urlBase + '/allumer/feux/vert').then(donnees => {
                    this.props.navigation.push('LancementPenalite', { headerTitle: this.props.route.params.headerTitle, texte: 'Durée des feux verts :', page: 3, temps: this.props.route.params.tempsFeuxVert, mode: this.props.route.params.mode });
                });
            });
        } else {
...
export default LancementPenalite;